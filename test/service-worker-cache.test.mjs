...
function chargerWorker(caches, networkFetch = fetch) {
  const handlers = {};
  const self = {
    location: { origin: "https://yole.example" },
    clients: { claim: async () => {} },
    skipWaiting: async () => {},
...
test("l'installation précache tout CORE en une transaction et attend la prochaine ouverture", async () => {
  ...
  assert.doesNotMatch(source, /self\.skipWaiting\s*\(/, "une mise à jour ne doit pas prendre un onglet en cours");
...
test("les huit pistes réellement utilisées sont présentes dans CORE et sur disque", async () => {
  ...
  for (const musique of musiques) {
    assert.ok(core.includes(musique), `${musique} absente du précache`);
...
test("une piste précachée répond correctement aux requêtes Range hors ligne", async () => {
