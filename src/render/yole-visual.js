...
// au lieu de 42°/52°.
const CREW_TORSO_RECLINE_ASTRIDE = 42;
// Dos à la mer et penché en arrière, l'homme assis à cheval prend le bois
// DERRIÈRE ses hanches, les deux mains du côté du large (photos 1 et 3) :
// la main d'appui loin, la seconde près du bassin. En mètres le long de la
// perche, depuis le bassin, vers le large.
const CREW_GRIP_BEHIND_FAR = 0.32;
const CREW_GRIP_BEHIND_NEAR = 0.11;
...
export class CrewVisual {
  ...
  applyRigContacts(hike, seated, catchLoad, transferCrouch) {
    ...
    if (legPose === "bas") {
      this.reclineTorso(CREW_TORSO_RECLINE_ASTRIDE, legAuthority, -versCoque, false);
    ...
    // Le tronc part en arrière, le regard reste sur la voile : la nuque se
    // redresse vers la verticale et vers la coque. Sans ça, l'homme fixe le
    // ciel — vu sur la planche de poses après le retournement.
    if (legPose === "bas" || legPose === "bateau") this.gazeAtSail(legAuthority, versCoque);
    ...
    // Assis à cheval, dos à la mer : les deux prises passent DERRIÈRE les
    // hanches, côté large. Allongé ou pieds au plat-bord, l'homme tient le
    // bois de part et d'autre du bassin comme avant.
    const versLeLarge = Math.sign(this.root.position.x) || 1;
    const prisesDerriere = legPose === "bas";
    const handDx = prisesDerriere
      ? versLeLarge * CREW_GRIP_BEHIND_FAR
   ...
   */
  gazeAtSail(weight, versCoque) {
    if (!this.rigRoot?.updateWorldMatrix || !this.rigJoints || weight <= 0.001) return false;
    if (this.gazeNeck === undefined) {
    ...
    this.legOutward.normalize();
    this.legDir.set(0, 1, 0).addScaledVector(this.legOutward, CREW_GAZE_INBOARD).normalize();
  ...
  update(time, dt, x, z, velocity, roll, impact, active, stumble = 0, boostForward = 0, boostSide = 0, cadence = 0, bail = 0, drink = 0, cohesion = 1, shiftMotion = null) {
    ...
    // Dans ce repère retourné, +Z local pointe vers la COQUE : renverser le
    // buste au-dessus de l'eau est donc un tangage NÉGATIF du bassin. Le signe
    // est porté ici, une fois : tête et nuque compensent dans l'autre sens
    // (le regard reste sur la voile), les jambes filles du bassin remontent
    // toujours vers la coque, la prise de main s'allonge un peu.
    ...
    let grip = hike * CREW_GRIP_REACH * familyPose.grip * (1 - desordre * 0.30) - recline;
